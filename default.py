...
@plugin.route("/")
def index():
    _all_artists = ListItem("Artists")
    _all_artists.setArt({"fanart": FANART, "icon": os.path.join(MEDIA_DIR, "artists.jpg")})
    addDirectoryItem(plugin.handle, plugin.url_for(musicmp3_artist_main), _all_artists, True)
...
def musicmp3_artist_main():
    _directory_items = []
    for i, gnr in enumerate(gnr_ids):
        li = ListItem("{0} Artists".format(gnr[0]))
        li.setArt(
            {
                "fanart": FANART,
                "icon": os.path.join(
...
def musicmp3_artists_gnr(gnr):
    ...
    for sub_gnr in gnr_ids[int(gnr)][1]:
        ...
        li = ListItem("{0} Artists".format(sub_gnr[0]))
        li.setArt(
            {
                "fanart": FANART,
                "icon": os.path.join(
...
def musicmp3_main_albums(section, gnr_id, sort, index):
    ...
    if len(albums) >= dir_items:
        next_index = str(index + dir_items)
        li = ListItem("More {0}+".format(next_index))
...
def musicmp3_main_artists(gnr_id, index):
    ...
    #context_menu = []
        ...
        #next_index = str(index + dir_items)
        #next_page = "ActivateWindow(Music,{0})".format(plugin.url_for(musicmp3_main_artists, gnr_id, next_index))
        #context_menu.append(("Next {0}+".format(next_index), next_page))
    #previous_index = str(index - dir_items)
    #if int(previous_index) >= 0:
        #previous_page = "ActivateWindow(Music,{0})".format(plugin.url_for(musicmp3_main_artists, gnr_id, previous_index))
        #context_menu.append(("Previous {0}+".format(previous_index), previous_page))
